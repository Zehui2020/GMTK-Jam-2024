...
public class Crow3Entity : BaseEntity
{
    ...
    public override void HandleActiveTrait(float _scaleAngle)
    {
        ...
        if (hasAbilityActivated)
        {
            ...
            activeMovementValue = isEnemy ?
                (_scaleAngle <= -5 ? 0.3f : 1.0f) : // check if tilted to enemy side if is enemy
                (_scaleAngle >= 5 ? 0.3f : 1.0f); //check if tilted to ally side if is ally
